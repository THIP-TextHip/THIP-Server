...
export default function (data) {
    ...
    if(__VU === 1) {
        const expectedCount = data.likeStatus.filter(s => s).length;

        // 피드 상세조회 api
        let countRes = http.get(`${BASE_URL}/feeds/${FEED_ID}`, params);

        if (countRes.status === 200) {
            let body = JSON.parse(countRes.body);
            let actualCount = body.data.likeCount;
            if (actualCount !== expectedCount) {
                console.error(`Like count mismatch! expected=${expectedCount}, actual=${actualCount}`);
            }
        }
    }

    sleep(0.01); // 사용자가 좋아요 버튼을 동시에 연타한다고 가정 10ms
}
