...
class RoomSearchApiTest {
    ...
    @Test
    @DisplayName("isAllCategory=true, keyword 입력 x, 카테고리 선택 X -> 방 전체조회")
    void search_allCategory_true_keyword_empty_category_null() throws Exception {
        ...
        ResultActions result = mockMvc.perform(get("/rooms/search")
                ...
                .param("isAllCategory", "true") // 전체 카테고리 포함
                .param("sort", "deadline")
    ...
    @Test
    @DisplayName("isAllCategory=true, keyword = [과학], 카테고리 선택 X -> 전체 방중에서 키워드 검색")
    void search_allCategory_true_keyword_nonEmpty_category_null() throws Exception {
        // given
        RoomJpaEntity science_room_1 = saveScienceRecruitingRoom("과학-책", "과학-방-1일뒤-활동시작", LocalDate.now().plusDays(1), LocalDate.now().plusDays(30));
        updateRoomMemberCount(science_room_1, 4);
        ...
        ResultActions result = mockMvc.perform(get("/rooms/search")
                .requestAttr("userId", 1L)
                .param("keyword", "과학")   // 실제 키워드 있음
                .param("isAllCategory", "true")
                .param("sort", "deadline")
                .param("isFinalized", "false"));
        ...
        result.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.data.roomList[0].roomName", containsString("과학")))
                .andExpect(jsonPath("$.data.roomList[1].roomName", containsString("방이름입니다")))
                .andExpect(jsonPath("$.data.roomList[2].roomName", containsString("과학")))
                .andExpect(jsonPath("$.data.isLast", is(true)));
    }

    @Test
    @DisplayName("isAllCategory=true,  keyword 입력 x, 카테고리 = [과학/IT] -> [과학/IT] 카테고리 내 전체 조회")
    void search_allCategory_true_keyword_empty_category_given() throws Exception {
        // given
        RoomJpaEntity science_room_1 = saveScienceRecruitingRoom("과학-책", "과학-방-1일뒤-활동시작", LocalDate.now().plusDays(1), LocalDate.now().plusDays(30));
        updateRoomMemberCount(science_room_1, 4);
        ...
        ResultActions result = mockMvc.perform(get("/rooms/search")
                .requestAttr("userId", 1L)
                .param("category", Category.SCIENCE_IT.getValue()) // 과학 카테고리 설정
                .param("isAllCategory", "true")
                .param("sort", "deadline")
                .param("isFinalized", "false"));
