// message/adapter/out/event/dto/RoomEvents.java
...
public class RoomEvents {
    ...
    @Builder
    public record RoomVoteStartedEvent(Long targetUserId, Long roomId, String roomTitle,
                                       Integer page, Long postId) {}

    // 내가 참여한 모임방에 새로운 기록이 작성된 경우
    @Builder
    public record RoomRecordCreatedEvent(Long targetUserId, Long actorUserId, String actorUsername,
                                         Long roomId, String roomTitle, Integer page, Long postId) {}

    // 내가 참여한 모임방이 조기 종료된 경우 (호스트가 모집 마감 버튼 누른 경우)
    @Builder
    public record RoomRecruitClosedEarlyEvent(Long targetUserId, Long roomId, String roomTitle) {}

    // 내가 참여한 모임방 활동이 시작된 경우 (방이 시작 기간이 되어 자동으로 시작된 경우)
    @Builder
    public record RoomActivityStartedEvent(Long targetUserId, Long roomId, String roomTitle) {}

    // 내가 방장일 때, 새로운 사용자가 모임방 참여를 한 경우
    @Builder
    public record RoomJoinRequestedToOwnerEvent(Long ownerUserId, Long roomId, String roomTitle,
